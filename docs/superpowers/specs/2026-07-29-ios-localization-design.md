...
- **布局稳定性**：语言切换后界面不得错位、截断或互相挤压。翻译时控制文案长度；按钮/标签用可伸缩布局（`frame(maxWidth:)`、多行 `lineLimit`/`minimumScaleFactor` 按需）；不依赖某一语种的固定字宽做对齐。四语都要过一遍首页、编辑、处理、设置页的视觉检查。
...
7. 四语切换后首页 / 编辑 / 处理 / 设置页布局不错位、不截断关键控件。
