...
- [ ] 等待银行与付费 App 协议审核通过（当前付费协议：正在处理）
- [ ] 开启 Family Sharing（已确认要开；ASC 上需再确认一次「开启」，开启后不能关闭）
- [ ] 完成欧盟 DSA 交易商声明与联系信息核验（要上欧盟必须做）
- [ ] 使用 Sandbox Apple Account 和 TestFlight 验证购买

### App Store Connect 进度记录（2026-07-29）

| 项 | 状态 / 值 |
| --- | --- |
| Team | tao sun / 48TLUK3DQ8 |
| App | 镜隐 · `6795803353` · Bundle `com.reaidea.jingyin` |
| IAP | `com.reaidea.jingyin.lifetime` · `6795803473` · 准备提交 |
| IAP 价格 | US $4.99；CN ¥28（手动）；其余自动 |
| App 价格 | Free $0.00 |
| 免费 App 协议 | 有效 |
| 付费 App 协议 | 已签署 → 正在处理（税银提交后） |
| 银行 | China Merchants Bank · CNY · 正在处理 |
| W-8BEN | 使用中（2026-07-29） |
| Foreign Status 证明 | 使用中（2026-07-29） |
| 国务院 810 | 有效 |
| Family Sharing | 待在 IAP 页最终确认开启 |
| DSA | 未完成；上欧盟需选交易商并核验地址/电话/邮箱 |

## P0：可控遮盖

- [ ] 建立 `MaskTrack`、归一化坐标和关键帧数据结构
- [ ] 当前画面检测多张人脸并逐个勾选
- [ ] 椭圆和矩形手动蒙版
- [ ] 拖动、缩放和删除蒙版
- [ ] 单个蒙版覆盖范围扩大/缩小
...
- [ ] 区分“视觉模糊”和“隐私级像素化”
- [ ] 导出时清除位置、设备和拍摄时间等元数据
- [ ] 离开项目、取消、失败和完成后清理输入副本与临时文件
- [ ] 统一 README、界面和实际视频时长限制
- [ ] 免费/买断导出均覆盖原声、静音和变音
...
- [ ] 隐私政策与 App Privacy 填写
- [ ] 简中、繁中、英文、日文商店文案
- [ ] 免费版与永久版截图
- [ ] TestFlight 内测
- [ ] App Review Notes 说明完全本地处理和 IAP 测试路径
