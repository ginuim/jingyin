...
- [x] 四工具单面板、固定下一步、当前帧对象对应、缩略时间轴与共享全屏区域控制。
- [x] 手动区域默认固定位置；显式开启随时间调整；时段修改不生成位置记录；会话撤销与旧数据兼容通过模型验证。
- [x] 按用户反馈移除重复主体入口、收紧间距和圆角；连续寻帧与预览检测刷新已接入。
- [ ] 完整导出/取消/失败重试、真机性能与四语无障碍矩阵，详见 [验证记录](./ios-video-refactor-validation-2026-09-09.md)。原有商店与真机未完成项保持未完成。

### 下一对话执行顺序（2026-09-09）

- [ ] 先收尾视频：取消/完成竞争、失败重试、连续寻帧性能、手动区域输出一致性、四语与无障碍适配。
