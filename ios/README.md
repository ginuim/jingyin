...
- 相册多选照片、顺序本地识别和批量导出
- 编辑页实时显示遮盖效果
- 精细人物轮廓分割、独立人脸遮盖、猫狗区域识别、背景或全画面遮盖
- 模糊、像素化和可自定义颜色的 ASCII 字符画效果、三档推理频率
- 原声、变音（可调半音、实时试听）或静音
- 导出分辨率与帧率选择、进度、稳定的剩余时间估算、取消和失败重试
- StoreKit 2 永久解锁、恢复购买；免费版：视频前 30 秒 / 720p，照片每次 1 张；买断解锁完整视频与照片批量（照片能力上架前须落地，见 `docs/ios-launch-todo.md`）
- 5 分钟/1 GB 视频限制、存储空间检查和精细档自动降级
- 导出预览、保存相册、系统分享

照片模式会把每个自动识别到的人物、人脸或猫狗保存为独立蒙版组，而不是提前
合并成一张不可编辑的位图。人物使用 Vision 人物实例分割，并与独立的人体语义
分割结果求交，过滤把背景识别为人物和少量边缘离群像素的情况；人脸和猫狗把
检测框与通用前景实例匹配，从共享标签图提取各自的像素边缘。每组只记录实例编号，
不会为多人照片重复保存多份全尺寸蒙版。用户可以逐张点选、删除、拖动或缩放
任意一组，也可以补充椭圆和矩形蒙版。设备不支持实例分割或匹配失败时会回退
到扩大后的安全区遮罩，不会直接漏掉主体。

免费版每次最多导出当前 1 张照片，永久版解锁整批导出；处理器内部会再次强制
该边界。照片按顺序解码和渲染，导出统一清除位置、设备、拍摄时间和方向标签。

定价：当前售价 ¥28 / $4.99；美国目标 $9.99（稳定后再改）；大陆先观察。见 `docs/app-store-market-and-pricing.md`。

猫狗当前使用 Vision 区域识别。人物使用 Vision 语义分割；精细档会再用前景
...
完成端到端测试：导入、人物与猫狗识别、H.264/AAC 导出和结果预览均成功，
