...
- 产品是端侧视频与照片隐私处理工具：原素材、识别数据和导出结果不上传服务器。
- 人脸相关数据未来可以写入设备本地的持久化偏好（例如 UserDefaults），但仍永久禁止上传、同步或传出服务器。
- 任何改变人脸数据收集、派生、存储、保留、删除或共享方式的功能，必须在同一变更中同步更新四语隐私政策（`landingpage/src/i18n/privacy/`）、App Store Connect 审核说明和相关文档；不能只改代码或 UI。
- 首发重点是短视频可靠完成 + 照片批量打码；不承诺十几分钟 vlog、后台处理、4K/HDR 保留或中断恢复；人脸贴纸后置。
- 付费模型是免费下载安装 + Non-Consumable 一次买断，不做订阅、不加广告。
- 永久版 Product ID 固定为 `com.reaidea.jingyin.lifetime`。
- 免费用户可以完整编辑和预览；视频只能导出前 30 秒、最高 720p、无水印；照片每次只能导出 1 张。买断后解除这些限制（完整视频导出 + 照片批量）。
- 当前售价：中国大陆 ¥28（手动覆盖，先观察）、美国等 $4.99。美国目标价 $9.99，仅在视频与照片批处理稳定后再永久改价，不排自动涨回；大陆不预设涨价。细则见 `docs/app-store-market-and-pricing.md`。改价格或付费边界时必须同步更新该文档与 `docs/ios-launch-todo.md`，不能只改 UI。
- 上架前必须完成照片批量打码；可控遮盖里仍优先保证手动蒙版与导出坐标一致，指定人脸跟踪达标前可继续隐藏。
...
- 免费版限制必须在视频 / 照片处理器内部再次强制，不能只依赖导出设置页隐藏选项。
...
2. 可控遮盖：`MaskTrack` 数据模型、手动形状、拖动/缩放、关键帧；指定人脸跟踪达标后再开放。
3. StoreKit 2：买断、免费视频 30 秒/720p 与照片单张、恢复购买、Sandbox/TestFlight 验证。
4. 隐私表达：默认推荐像素化、区分视觉模糊和隐私级像素化、清除导出 metadata。
5. 照片批量打码：多选、顺序处理、统一设置、逐张复核、免费 1 张 / 买断批量（上架硬门槛）。
6. 真机矩阵测试和商店提交材料。
7. 人脸贴纸（后置，不阻塞上架）。
